...
{
    // Ground DETECT.
    PIN_PIO_PORT->DR_CLEAR = (PIN_DETECT_MASK);

    // Set TCK, TMS, TDI GPIO outputs to high.
    PIN_PIO_PORT->DR_SET = (PIN_TCK_SWCLK_MASK | PIN_TMS_SWDIO_MASK | PIN_TDI_MASK);

    // Switch TCK, TMS, TDI to outputs.
    PIN_PIO_PORT->GDIR |= (PIN_TCK_SWCLK_MASK | PIN_TMS_SWDIO_MASK | PIN_TDI_MASK);

    // Switch TDO_SWO to GPIO input (TDO).
    PIN_PIO_PORT->GDIR &= ~(PIN_TDO_SWO_MASK);

    // Enable TMS translator output.
    PIN_PIO_PORT->DR_SET = (PIN_TMS_SWDIO_TXEN_MASK);
}
...
__STATIC_INLINE void PORT_SWD_SETUP(void)
{
    ...
    PIN_PIO_PORT->DR_SET = (PIN_TCK_SWCLK_MASK | PIN_TMS_SWDIO_MASK);

    // Set SWCLK and SWDIO to outputs.
    PIN_PIO_PORT->GDIR |= (PIN_TCK_SWCLK_MASK | PIN_TMS_SWDIO_MASK);

    // Set TDI to input.
    PIN_PIO_PORT->GDIR &= ~(PIN_TDI_MASK);
...
__STATIC_INLINE void PORT_OFF(void)
{
    ...
    PIN_PIO_PORT->DR_CLEAR =   PIN_TMS_SWDIO_TXEN_MASK
                             | PIN_RESET_TXEN_MASK;

    // Disable SWCLK and TDI (set to inputs).
    PIN_PIO_PORT->GDIR &= ~(PIN_TCK_SWCLK_MASK | PIN_TDI_MASK);
    ...
    // Release DETECT.
    PIN_PIO_PORT->DR_SET = (PIN_DETECT_MASK);
}
...
{
    return (((PIN_PIO_PORT->DR) & PIN_TCK_SWCLK_MASK) ? 1 : 0);
}
...
{
    PIN_PIO_PORT->DR_SET = (PIN_TCK_SWCLK_MASK);
}
...
{
    PIN_PIO_PORT->DR_CLEAR = (PIN_TCK_SWCLK_MASK);
}
...
{
    return (((PIN_PIO_PORT->DR) & PIN_TMS_SWDIO_MASK) ? 1 : 0);
}
...
{
    PIN_PIO_PORT->DR_SET = (PIN_TMS_SWDIO_MASK);
}
...
{
    PIN_PIO_PORT->DR_CLEAR = (PIN_TMS_SWDIO_MASK);
}
...
{
    PIN_PIO_PORT->GDIR |= PIN_TMS_SWDIO_MASK;   /* SWDIO Pin Switch to Output mode */
    PIN_PIO_PORT->DR_SET = PIN_TMS_SWDIO_TXEN_MASK;
}
...
{
    PIN_PIO_PORT->DR_CLEAR = PIN_TMS_SWDIO_TXEN_MASK;
    PIN_PIO_PORT->GDIR &= ~PIN_TMS_SWDIO_MASK; /* SWDIO Pin Switch to Input mode */
}
...
{
    return (((PIN_PIO_PORT->DR) & PIN_TDI_MASK) ? 1 : 0);
}
...
{
    if (bit & 0x01) {
        PIN_PIO_PORT->DR_SET = (PIN_TDI_MASK);
    } else {
        PIN_PIO_PORT->DR_CLEAR = (PIN_TDI_MASK);
    }
}
...
{
    return (((PIN_PIO_PORT->DR) & PIN_TDO_SWO_MASK) ? 1 : 0);
}
...
{
    return (((PIN_PIO_PORT->DR) & PIN_RESET_MASK) ? 1 : 0);
}
...
__STATIC_FORCEINLINE void     PIN_nRESET_OUT(uint32_t bit)
{
    ...
    if (bit & 0x01) {
        // Change pin to input.
        PIN_PIO_PORT->GDIR &= ~(PIN_RESET_MASK);
        // Disable level translator output.
        PIN_PIO_PORT->DR_CLEAR = (PIN_RESET_TXEN_MASK);
        // Set RESET to high. (Probably not be necessary)
        PIN_PIO_PORT->DR_SET = (PIN_RESET_MASK);
    }
    else {
        // Set RESET low.
        PIN_PIO_PORT->DR_CLEAR = PIN_RESET_MASK;
        // Enable level translator output to drive.
        PIN_PIO_PORT->DR_SET = (PIN_RESET_TXEN_MASK);
...
__STATIC_INLINE void DAP_SETUP(void)
{
  ...
  /* GPIO configuration of TMS_SWDIO on GPIO_AD_B1_12 (pin H12) */
  gpio_pin_config_t TMS_SWDIO_config = {
      .direction = kGPIO_DigitalOutput,
      .outputLogic = 1U,
      .interruptMode = kGPIO_NoIntmode
  };
  /* Initialize GPIO functionality on GPIO_AD_B1_12 (pin H12) */
  GPIO_PinInit(GPIO1, 28U, &TMS_SWDIO_config);

  /* GPIO configuration of TDI on GPIO_AD_B1_13 (pin H11) */
  ...
  GPIO_PinInit(GPIO1, 29U, &TDI_config);

  /* GPIO configuration of TDO_SWO on GPIO_AD_B1_14 (pin G12) */
  gpio_pin_config_t TDO_SWO_config = {
      .direction = kGPIO_DigitalInput,
      .outputLogic = 0U,
      .interruptMode = kGPIO_NoIntmode
  };
  /* Initialize GPIO functionality on GPIO_AD_B1_14 (pin G12) */
  GPIO_PinInit(GPIO1, 30U, &TDO_SWO_config);

  /* GPIO configuration of TCK_SWCLK on GPIO_AD_B1_15 (pin J14) */
  gpio_pin_config_t TCK_SWCLK_config = {
      .direction = kGPIO_DigitalOutput,
      .outputLogic = 1U,
      .interruptMode = kGPIO_NoIntmode
  };
  /* Initialize GPIO functionality on GPIO_AD_B1_15 (pin J14) */
  GPIO_PinInit(GPIO1, 31U, &TCK_SWCLK_config);
  ...
  IOMUXC_SetPinConfig(IOMUXC_GPIO_AD_B1_12_GPIO1_IO28, 0x0190B0U);
  IOMUXC_SetPinConfig(IOMUXC_GPIO_AD_B1_13_GPIO1_IO29, 0x0190B0U);
  IOMUXC_SetPinConfig(IOMUXC_GPIO_AD_B1_14_GPIO1_IO30, 0x0190B0U);
  IOMUXC_SetPinConfig(IOMUXC_GPIO_AD_B1_15_GPIO1_IO31, 0x0190B0U);

    // Configure GPIO outputs.
    PIN_PIO_PORT->DR_CLEAR = ( PIN_TMS_SWDIO_TXEN_MASK   // Disable TMS/SWDIO drive.
                             | PIN_RESET_TXEN_MASK );    // Disable RESET drive.

    // Set GPIO directions.
    PIN_PIO_PORT->GDIR |= ( PIN_TMS_SWDIO_TXEN_MASK
                          | PIN_RESET_TXEN_MASK
                          | PIN_DETECT_MASK );

    PIN_PIO_PORT->GDIR &= ~( PIN_TCK_SWCLK_MASK
                           | PIN_TMS_SWDIO_MASK
                           | PIN_TDO_SWO_MASK
                           | PIN_TDI_MASK
                           | PIN_RESET_MASK );
